...
class WeChatCloud(BaseWeChatAPI):
    ...
    async def invoke_cloud_function(self, env, name, data=None):
        """
        触发云函数
    ...
    async def db_import(self, env, collection, file_path, file_type, conflict_mode, stop_on_error=True):
        """
        数据库导入
    ...
    async def db_export(self, env, file_path, file_type, query):
        """
        数据库导出
    ...
    async def db_query_migrate_info(self, env, job_id):
        """
        数据库迁移状态查询
    ...
    async def db_update_index(self, env, collection, create_indexes=None, drop_indexes=None):
        """
        变更数据库索引
    ...
    async def db_add_collection(self, env, collection):
        """
        新增集合
    ...
    async def db_delete_collection(self, env, collection):
        """
        删除集合
    ...
    async def db_get_collection(self, env, offset=0, limit=10):
        """
        获取特定云环境下集合信息
    ...
    async def db_add(self, env, query):
        """
        数据库插入记录
    ...
    async def db_delete(self, env, query):
        """
        数据库删除记录
    ...
    async def db_update(self, env, query):
        """
        数据更新记录
    ...
    async def db_query(self, env, query):
        """
        数据库查询记录
    ...
    async def db_aggregate(self, env, query):
        """
        数据库聚合
    ...
    async def db_count(self, env, query):
        """
    ...
    async def upload_file(self, env, path):
        """
        获取文件上传链接
    ...
    async def download_files(self, env, file_list):
        """
        获取文件下载链接
    ...
    async def delete_files(self, env, fileid_list):
        """
        删除文件
    ...
    async def get_qcloud_token(self, lifespan=7200):
        """
        获取腾讯云 API 调用凭证
        ...
        """
        return await self._post("tcb/getqcloudtoken", data={"lifespan": lifespan})
