...
class WeChatMedia(BaseWeChatAPI):
    ...
    async def upload(self, media_type, media_file):
        """
        上传临时素材
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("media/upload", params={"type": media_type}, files={"media": media_file})

    async def upload_img(self, image_file):
        """
        上传永久图片
        ...
        :return: 返回的 JSON 数据包
        """
        return await self._post("media/uploadimg", files={"media": image_file})

    async def get_url(self, media_id):
        """
        获取临时素材
    ...
    async def get_jssdk_url(self, media_id):
        """
        获取高清语音素材
    ...
    async def download(self, media_id):
        ...
        """
        return await self._get(await self.get_url(media_id))
